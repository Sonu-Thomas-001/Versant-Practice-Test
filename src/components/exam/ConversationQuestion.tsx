...
import { Volume2, Play, Mic, CheckCircle, Loader2 } from 'lucide-react';
...
export function ConversationQuestion({ question, onAnswer, initialAnswer = '', onAutoNext }: QuestionProps) {
  ...
  // Clean the prompt to remove newlines and dashes for better text-to-speech reading
  const cleanPrompt = question.prompt
    .replace(/\\n/g, '. ')
    .replace(/\n/g, '. ')
    .replace(/-/g, ' ')
    .replace(/\s+/g, ' ')
  ...
  const startRecording = () => {
    ...
    // Stop recording automatically after fixed duration (e.g., 10 seconds for Part C)
    timerRef.current = setTimeout(() => {
      try { recognitionRef.current?.stop(); } catch(e) {}
      finishAndAdvance();
    }, 10000);
  ...
  return (
    <div className="space-y-8 mt-4">
      <div className="flex flex-col items-center justify-center space-y-6 py-12 px-4 bg-gray-50 border border-gray-200 rounded-xl text-center transition-colors">
        
        {phase === 'idle' && (
          <div className="text-gray-500 flex flex-col items-center animate-pulse">
             <Loader2 className="w-12 h-12 mb-4 animate-spin" />
             <p className="text-lg font-medium">Preparing conversation...</p>
          </div>
        )}
        ...
        {phase === 'playing' && (
          <div className="text-blue-600 flex flex-col items-center">
             ...
             <p className="text-lg font-bold">Playing conversation & question...</p>
             {manualFallback && (
        ...
        {phase === 'recording' && (
          <div className="text-red-500 flex flex-col items-center">
            ...
            <p className="text-lg font-bold">Recording your answer...</p>
            <p className="text-sm text-gray-500 mt-2">Speak your response now</p>
          </div>
        )}
