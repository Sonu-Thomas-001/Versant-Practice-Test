...
export function BuildQuestion({ question, onAnswer, initialAnswer = '', onAutoNext }: QuestionProps) {
  ...
  const playAudio = (wordsToSpeak: string) => {
    setPhase('playing');
    setManualFallback(false);
  ...
  const startRecording = () => {
    ...
    // Stop recording automatically after fixed duration (e.g., 7 seconds for Part B)
    timerRef.current = setTimeout(() => {
      try { recognitionRef.current?.stop(); } catch(e) {}
      finishAndAdvance();
    }, 7000);
      ...
      <div className="flex flex-col items-center justify-center space-y-6 py-12 px-4 bg-gray-50 border border-gray-200 rounded-xl relative overflow-hidden transition-colors">
        ...
        {phase === 'playing' && (
          <div className="text-blue-600 flex flex-col items-center">
            ...
            <p className="text-lg font-bold">Playing jumble words...</p>
            {manualFallback && (
              <Button onClick={() => playAudio(question.jumbledWords?.join(', ') || '')} className="mt-6" variant="outline">
        ...
        {phase === 'recording' && (
          <div className="text-red-500 flex flex-col items-center">
            ...
            <p className="text-sm text-gray-500 mt-2">Speak your sentence now</p>
          </div>
        )}
